	...
	m_is_moving(false)
{
	...
	this->addOperation("resetPositionActual", &CartesianGeneratorPosDMP::resetPositionActual,
			this, OwnThread).doc("Reset generator's position");
	this->addOperation("setCartesianStiffness", &CartesianGeneratorPosDMP::setCartesianStiffness,
			this, OwnThread).doc("Set the stiffness to desired values");
	this->addOperation("startKinesthetic", &CartesianGeneratorPosDMP::start_kinesthetic,
			this, OwnThread).doc("Start kinesthetic guidance. mirror the actual pose in desired pose");
