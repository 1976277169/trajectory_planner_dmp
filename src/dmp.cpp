...
void DMP::calculate_one_step_dmp(float t){
   ...
   if (t==0){
      ...
      z=0;
      f=0;
//       cout<<"CALCULATE one step dmp is initialized"<<endl;
   }
   else{
//       cout<<"---------------begin printing data ------------------------------"<<endl;
      a=exp((alpha_v/dt)*(tau*T-t));
//       cout<<setw(12)<<"a "<<a<<" alpha_v "<<alpha_v<<" dt "<<dt<<" tau "<<tau<<" T "<<T<<" t "<<t<<endl;
...
//       cout<<"***** data in for loop ************"<<endl;
      for (int i=0; i<n; i++){
         ...
         sum_psi=sum_psi+psi;
         f=f+psi*w[i]*v;
//          cout<<"a "<<a<<" c[i] "<<c[i]<<" sigma[i] "<<sigma[i]<<" psi "<<psi<<" sum_psi "<<sum_psi<<" w[i] "<<w[i]<<" f "<<f<<endl;
         
      }
//       cout<<"***********************************"<<endl;
//       cout<<"a= "<<a<<" psi= "<<psi<<" sum_psi= "<<sum_psi<<" f= "<<f<<endl;
      f=f/sum_psi;      
//       cout<<"f "<<f<<endl;
      dz=(1/tau)*(alpha_z*(beta_z*(r-y)-z)+f/*+ inp_ico*/);/// 
//       cout<<"dz= "<<dz<<endl;
      z=z+dz;
//       cout<<"z= "<<z<<endl;
      dy=(1/tau)*z; 
      y=y+dy;
//       cout<<"y= "<<y<<" dy= "<<dy<<endl;
//       cout<<"---------------end of printing data ---------------------------"<<endl;
//       cout<<"---------------------------------------------------------------"<<endl;
   }
}
